...
<div align="center"  id="div1">
    ...
    <c:if test="${message.flag==1}">
        ...
        <a href="${pageContext.request.contextPath}/worker/allinfor?workerid=${workerid}&p=${page.firstPage}">第一页</a>
        <c:if test="${page.nextPage==0}">
            <a href="javascript:void(0)">下一页</a>
        </c:if>
        <c:if test="${page.nextPage!=0}">
            <a href="${pageContext.request.contextPath}/worker/allinfor?userid=${page.list[0].userid}&p=${page.nextPage}">下一页</a>
        </c:if>
        <c:if test="${page.prePage==0}">
            <a href="javascript:void(0)">上一页</a>
        </c:if>
        <c:if test="${page.prePage!=0}">
            <a href="${pageContext.request.contextPath}/worker/allinfor?userid=${page.list[0].userid}&p=${page.prePage}">上一页</a>
        </c:if>
        <a href="${pageContext.request.contextPath}/worker/allinfor?workerid=${workerid}&p=${page.lastPage}">最后页</a>
    </c:if>
</div    >
</body>
</html>
